...
class TestEditsAggregate(unittest.TestCase):
  ...
  def test_edits_aggregate_basic(self, mock_get):
    ...
    result = edits_aggregate("en.wikipedia.org", "daily", "20250101", "20250102")
  ...
  def test_edits_aggregate_with_custom_parameters(self, mock_get):
    ...
    mock_get.assert_called_once_with(
      "https://wikimedia.org/api/rest_v1/metrics/edits/aggregate/es.wikipedia.org/user/content/monthly/20250101/20250105",
      headers={"User-Agent": "wikiedits-api/0.1.0", "Accept": "application/json"},
      timeout=30,
    )
  ...
  def test_edits_aggregate_with_default_parameters(self, mock_get):
    ...
    expected_url = "https://wikimedia.org/api/rest_v1/metrics/edits/aggregate/fr.wikipedia.org/all-editor-types/all-page-types/daily/20250201/20250228"
    mock_get.assert_called_once_with(
      expected_url,
      headers={"User-Agent": "wikiedits-api/0.1.0", "Accept": "application/json"},
      timeout=30,
