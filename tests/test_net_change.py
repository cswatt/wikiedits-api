...
class TestNetChange(unittest.TestCase):
  ...
  def test_net_bytes_diff_aggregate_custom_parameters(self, mock_get):
    ...
    mock_get.assert_called_once_with(
      "https://wikimedia.org/api/rest_v1/metrics/bytes-difference/"
      "net/aggregate/de.wikipedia.org/user/content/monthly/20250101/20250105",
      headers={"User-Agent": "wikiedits-api/0.1.0", "Accept": "application/json"},
      timeout=30,
    )
  ...
  def test_net_bytes_diff_net_per_page_basic(self, mock_get):
    ...
    mock_get.assert_called_once_with(
      "https://wikimedia.org/api/rest_v1/metrics/bytes-difference/"
      "net/per-page/en.wikipedia.org/Python/all-editor-types/"
      "daily/20250101/20250102",
      headers={"User-Agent": "wikiedits-api/0.1.0", "Accept": "application/json"},
      timeout=30,
    )
  ...
  def test_net_bytes_diff_net_per_page_custom_parameters(self, mock_get):
    ...
    mock_get.assert_called_once_with(
      "https://wikimedia.org/api/rest_v1/metrics/bytes-difference/"
      "net/per-page/es.wikipedia.org/Machine_learning/user/"
      "monthly/20250101/20250105",
      headers={"User-Agent": "wikiedits-api/0.1.0", "Accept": "application/json"},
      timeout=30,
    )
